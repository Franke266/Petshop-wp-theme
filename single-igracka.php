...
$sNazivIgracke=$post->post_title;
$sCijena='Cijena: '.$igracka_cijena.' Kn';
$sArtikala="";
if($igracka_kolicina=="" || $igracka_kolicina==0)
{
    $sArtikala='<span style="color:red;"> nedostupno</span>';
}
else
{
    $sArtikala= '<span style="color:green;"> na skladištu </span><span style="color:black;">('.$igracka_kolicina.')</span>';   
}
...
if ( have_posts() )
{
    ...
    $sveboje=array();
    $Boje = wp_get_post_terms( $post->ID, 'boja_igracke' );
    if(sizeof($Boje)>0)
    {
        foreach ($Boje as $key) {
    ...
    $svimaterijali=array();
    $Materijali = wp_get_post_terms( $post->ID, 'materijal_igracke' );
    ...
    if($sMaterijal=="")
    {
        ...
        echo '<div class="row">
                ...
                <div class="col-md-6" style="text-align:;"><h3 class="nazivsingle">'.$sNazivIgracke.'</h3><h4 class="dodatnosingle">Boja: '.$sBoja.'</h4><h4 class="dodatnosingle">Materijal: '.$sMaterijal.'</h4><h5 class="cijenasingle">'.$sCijena.'</h5><input type="number" id="odabrana_kolicina" value="1" min="1"><button class="button">Dodaj</button><h6>'.$sKolicina.'</h6></div>
...
$(document).ready(function() {
    var dostupna_kolicina = "<?php echo $igracka_kolicina ?>";
            $('.button').click(function () {
                var broj_artikala=document.getElementById("odabrana_kolicina").value;
                var cijena = "<?php echo $igracka_cijena ?>";
